...
/** null = ej kontrollerad, false = OK, true = avvikelse */
export type StabilityCheckValue = boolean | null;
...
export interface MovementAssessmentScores {
  ...
  core: number | null;
  /** null om ingen stabilitetsdata registrerats */
...
export interface MovementAssessmentStickTest {
  transversalLeft: StabilityCheckValue;
  transversalRight: StabilityCheckValue;
  frontalLeft: StabilityCheckValue;
  frontalRight: StabilityCheckValue;
  sagital: StabilityCheckValue;
  shouldersLeft: StabilityCheckValue;
  shouldersRight: StabilityCheckValue;
  pelvicHip: StabilityCheckValue;
  notes: string;
}

export interface MovementAssessmentLungeTest {
  footLeft: StabilityCheckValue;
  footRight: StabilityCheckValue;
  kneeLeft: StabilityCheckValue;
  kneeRight: StabilityCheckValue;
  hipLeft: StabilityCheckValue;
  hipRight: StabilityCheckValue;
  upperBodyLeft: StabilityCheckValue;
  upperBodyRight: StabilityCheckValue;
  postureLeft: StabilityCheckValue;
  postureRight: StabilityCheckValue;
  notes: string;
}

export interface MovementAssessmentStability {
  /** 2 = trestegs (null/false/true); saknas = legacy boolean */
...
export function createEmptyStickTest(): MovementAssessmentStickTest {
  return {
    transversalLeft: null,
    transversalRight: null,
    frontalLeft: null,
    frontalRight: null,
    sagital: null,
    shouldersLeft: null,
...
/** Cyklar stabilitetsrad: null → false (OK) → true (avvikelse) → null */
export function cycleStabilityCheckValue(current: StabilityCheckValue): StabilityCheckValue {
