...
/** Stopwatch rest timer for WorkoutActive set logging. */
export default function RestTimer({ onTimeLogged, isCompact = false }: RestTimerProps) {
  ...
  const stopTimer = useCallback(() => {
    ...
    if (finalElapsed > 0) onTimeLogged?.(finalElapsed);
...
const styles = StyleSheet.create({
  ...
  timerText: { fontSize: 40, fontWeight: '700', fontVariant: ['tabular-nums'] },
  runningIndicator: { width: 8, height: 8, borderRadius: 4, marginTop: 8 },
  ...
  stopButtonText: { color: '#FFFFFF', fontSize: 17, fontWeight: '700' },
  resetButton: { backgroundColor: '#2C2C2E' },
  resetButtonText: { color: '#8E8E93', fontSize: 17, fontWeight: '600' },
  compactContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#1A1A1A',
  ...
  compactTime: { fontSize: 22, fontWeight: '700', fontVariant: ['tabular-nums'], flex: 1 },
  compactButton: { paddingVertical: 6, paddingHorizontal: 16, borderRadius: 8 },
  compactStartButton: { backgroundColor: '#34C759' },
  compactStopButton: { backgroundColor: '#FF3B30' },
  compactButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '700' },
});
