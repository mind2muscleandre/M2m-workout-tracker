...
interface WorkoutState {
  ...
  isFetchingWorkoutDetail: boolean;
  isCopyingWorkout: boolean;
}

interface WorkoutActions {
  fetchWorkouts: (clientId: string) => Promise<void>;
  fetchAllWorkouts: () => Promise<void>;
...
export const useWorkoutStore = create<WorkoutStore>((set, get) => ({
  ...
  fetchAllWorkouts: async () => {
    try {
      ...
      console.error('Fetch all workouts error:', (error as Error).message);
      throw error;
    } finally {
      set({ isFetchingWorkouts: false });
    }
  ...
  fetchWorkoutDetail: async (workoutId: string) => {
    try {
      const { activeWorkout: existing } = get();
      if (
        existing?.id === workoutId &&
        existing.workout_exercises.length > 0
      ) {
        return;
      }

      set({ isFetchingWorkoutDetail: true });
      ...
      set({ isFetchingWorkoutDetail: false });
    }
  },
  ...
  updateWorkoutNotes: async (workoutId: string, notes: string | null) => {
    try {
      const { data: updatedWorkout, error } = await supabase
        .from('workouts')
        .update({ notes })
      ...
      console.error('Update workout notes error:', (error as Error).message);
      throw error;
    }
  },
  ...
  addWorkoutExercisesBatch: async (workoutId, items) => {
    if (items.length === 0) {
      return;
    }
    ...
    if (error) {
      console.error(
        'Batch add workout exercises error:',
        (error as Error).message
      );
      throw error;
    }
