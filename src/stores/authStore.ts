...
const STAFF_ROLES = new Set(['pt', 'admin', 'moderator']);

function resolveCoachRole(
  ptRole: UserRole | undefined,
  platformRole: string | null | undefined
): UserRole {
  const platform = (platformRole ?? '').toLowerCase();
  if (platform === 'admin' || platform === 'moderator') {
    return 'admin';
  }
  return ptRole ?? 'pt';
}
...
const fetchAnyUserProfile = async (userId: string): Promise<User | null> => {
  const [ptProfile, platformProfile] = await Promise.all([
    fetchUserProfile(userId),
    fetchUserProfileFromAiScreening(userId),
  ]);

  if (ptProfile && platformProfile) {
    return {
      ...ptProfile,
      full_name: ptProfile.full_name || platformProfile.full_name,
      email: ptProfile.email || platformProfile.email,
      role: resolveCoachRole(ptProfile.role, platformProfile.role),
    };
  }

  return ptProfile ?? platformProfile;
};
