...
const WEB_FILE_PREP_TIMEOUT_MS = 10000;
...
const fetchBlobWithTimeout = async (uri: string, timeoutMs: number): Promise<Blob> => {
  const response = await withTimeout(fetch(uri), timeoutMs, 'FILE_PREP_TIMEOUT');
  if (!response.ok) {
    throw new Error('FILE_PREP_FAILED');
  }
  return withTimeout(response.blob(), timeoutMs, 'FILE_PREP_TIMEOUT');
};
...
const buildWebFormData = async (payload: UploadPayload, accessToken: string): Promise<FormData> => {
  ...
  const fileParts = await Promise.all(
    payload.files.map(async (file) => {
      const blob = await fetchBlobWithTimeout(file.uri, WEB_FILE_PREP_TIMEOUT_MS);
...
export const uploadPtScreening = async (
  ...
  for (let attempt = 1; attempt <= MAX_UPLOAD_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (error instanceof Error && error.message === 'FILE_PREP_TIMEOUT') {
        throw new Error('Det tog för lång tid att läsa en bildfil. Välj om bilden och försök igen.');
      }
      if (error instanceof Error && error.message === 'FILE_PREP_FAILED') {
        throw new Error('En bildfil kunde inte läsas. Välj om bilden och försök igen.');
      }
      if (isNetwork) {
