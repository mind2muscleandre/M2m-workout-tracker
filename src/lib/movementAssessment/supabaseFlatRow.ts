...
export function mobilityTestPoang(
  ...
  return scoreMobilityBilateral(test, lv, rv);
}
...
export function mobilitySidePoang(
  ...
  return value != null ? scoreMobilityTestValue(test, value as number) : null;
}
...
export function buildFlatAssessmentColumns(assessment: MovementAssessment): Record<string, FlatScalar> {
  ...
  out.stab_pinne_poang = scoreStickTest(st);
  out.stab_lunge_poang = scoreLungeTest(lu);
