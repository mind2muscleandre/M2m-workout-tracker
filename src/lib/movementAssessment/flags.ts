...
function mobilityBelowNormalFlags(mobility: MovementAssessmentMobility): AssessmentFlag[] {
  ...
  for (const t of MOBILITY_TESTS) {
    ...
    const check = (side: 'Vänster' | 'Höger', v: number | null) => {
      if (v == null) return;
      const isLowerBetter = t.direction === 'lower';
      const belowNormal = !isLowerBetter && v < t.min;
      const aboveNormal = isLowerBetter && v > t.max;
      if (!belowNormal && !aboveNormal) return;

      const gap = isLowerBetter ? v - t.max : t.min - v;
      const severity = Math.min(5, Math.max(1, Math.ceil(gap / 10) + 2));
      const normalLabel = isLowerBetter ? `mål ≤${t.max}` : `normal ${t.min}–${t.max}`;
      out.push({
        id: `mobility_${t.key}_${side === 'Vänster' ? 'L' : 'R'}`,
        label: `${t.labelSv} (${side}) ${isLowerBetter ? 'över normalområde' : 'under normalområde'}`,
        severity,
        category: 'mobility',
        impactHint: `${v} (${normalLabel})`,
      });
    };
    check('Vänster', lv);
    check('Höger', rv);
